...
public class ConstServiceImpl extends CoreLocalService implements ConstService {
    ...
    @Override
    public VersionSimpleDTO getNewestVersion(VersionSimpleQuery query, Current current) {
        long t = TraceUtils.enter(log,"getNewestVersion",query);

        CoreBuffer buffer = bufferConfig.getBuffer();
        ...
        if (query != null){
            serviceQuery.setVersionName(query.getService());
        }
        List<VersionDTO> serviceList = listVersion(serviceQuery,current);
        ...
        List<VersionDTO> clientList = listVersion(clientQuery,current);
        VersionDTO client = ObjectUtils.getFirst(clientList);

        //设置返回值
        VersionSimpleDTO version = new VersionSimpleDTO();
        if (service != null) {
            version.setService(service.getVersionName());
        }
        if (client != null) {
            version.setClient(client.getVersionName());
        }

        TraceUtils.exit(log,"getNewestVersion",t,version);
        return version;
    }
    ...
    public List<VersionDTO>  listVersion(@NotNull VersionQuery query, Current current){
        ...
        CoreBuffer buffer = bufferConfig.getBuffer();
        List<VersionDTO> list = buffer.getList(key,aliveTime);
    ...
    private List<ConstEntity> listConst(@NotNull ConstQuery query, Current current){
        ...
        String key = JsonUtils.obj2CleanJson(query) + suffix;
        CoreBuffer buffer = bufferConfig.getBuffer();
        List<ConstEntity> list = buffer.getList(key,aliveTime);
        if (ObjectUtils.isEmpty(list)){
