...
module zeroc {
    interface StorageService {
        FullNodeDTO createMirror(FullNodeDTO src,UpdateNodeDTO request) throws CustomException; //建立镜像
        ...
        bool deleteNodeById(string id) throws CustomException; //删除树节点

        SimpleNodeList listNode(QueryNodeDTO query) throws CustomException; //查询节点信息

        FullNodeDTO getNodeInfo(SimpleNodeDTO node,QueryFullNodeDTO request) throws CustomException; //查询节点详细信息

        ["deprecate:使用listNode代替"] SimpleNodeList listAllNode(string userId); //获取指定账号所有节点
